...
export default class Lamborghini_Hurracan {
  ...
  setupFrontLight() {
    ...
    this.chassis.traverse((child) => {
      ...
      if (child.name === "Object_275" || child.name === "Object_268") {
  ...
  setupBrakeLight() {
    ...
    this.chassis.traverse((child) => {
      if (child.name === "Object_390" || child.name === "Object_381") {
        ...
        const flare1 = new THREE.Sprite(flareMaterial1);
        flare1.scale.set(1.5, 1, 1);
        flare1.visible = false;

        // SECOND FLARE - Secondary flare (larger)
        const flare2 = new THREE.Sprite(flareMaterial2);
        flare2.scale.set(1.5, 1, 1); // Larger than first flare
        flare2.visible = false;
  ...
  setupReverseLight() {
    ...
    this.chassis.traverse((child) => {
      if (child.name === "Object_394" || child.name === "Object_385") {
    ...
    this.reverseLights.forEach((light, index) => {
      if (this.reverseLightFlares[index]) {
        ...
        if (flare1.parent === this.chassis) {
          ...
          // Update both flares with their respective offsets
          const offset1 = new THREE.Vector3(1.15, 0, -2.9);
          const offset2 = new THREE.Vector3(-1.15, 0, -2.9);
  ...
  loadModels() {
    ...
    gltfLoader.load("./car/Lamborghini_Hu.glb", (gltf) => {
