...
export default class AIInactiveCar {
    constructor(scene, world, targetCar, startPosition = { x: 10, y: 4, z: 10 } , startRotation = { x: 0, y: 0, z: 0 }, loadingManager) {
        this.scene = scene;
        this.world = world;
    ...
    this.chassisModelPos = {
      ...
      z: 0,
        ...
        this.policeLightsSetup = false;
            ...
            // Add this new property for speed-based activation
    this.speedActivationEnabled = true; // Set to false if you want to disable speed activation
    this.speedActivationThreshold = 180; // Speed threshold for activation
    this.speedCheckInterval = 100; // Check speed every 100ms
...
setupPoliceLight() {
        ...
        const lightPositions = [
            { x: -0.5, y: 0.5, z: -0.5, isRed: false }, // Left blue light
            { x: 0.5, y: 0.5, z: -0.5, isRed: true },   // Right red light
            { x: -0.3, y: 0.5, z: -0.2, isRed: false }, // Additional left blue
            { x: 0.3, y: 0.5, z: -0.2, isRed: true }    // Additional right red
        ];
    ...
    setupBrakeLight() {
        ...
        this.chassis.traverse((child) => {
            if (child.name === "Object_12") {
                ...
                const flare1 = new THREE.Sprite(flareMaterial1);
                flare1.scale.set(1.2, 1.2, 1.2);
                flare1.visible = false;

                // SECOND FLARE - Secondary flare (larger)
                const flare2 = new THREE.Sprite(flareMaterial2);
                flare2.scale.set(1.2, 1.2, 1.2); // Larger than first flare
                flare2.visible = false;
        ...
        if (this.brakeLights.length === 0) {
            console.warn(
                "Brake lights not found in chassis model"
            );
        } else {
    ...
    updateBrakeLightFlarePositions() {
        ...
        this.brakeLights.forEach((light, index) => {
          ...
          const offset1 = new THREE.Vector3(0.8, -0.3, -3.2);
          const offset2 = new THREE.Vector3(-0.8, -0.3, -3.2);
    ...
    loadModels() {
        ...
        gltfLoader.load("./car/Police_Mustang_P.glb", gltf => {
        ...
        this.wheels = [];
        for (let i = 0; i < 4; i++) {
            gltfLoader.load("./car/Police_Mustang_Wheel.glb", gltf => {
...
loadAudio() {
    ...
    this.audio1 = new Audio('./sounds/police_siren.mp3'); // Your audio file path
    this.audio2 = new Audio('./sounds/policecar_sound.mp3'); // Your audio file path
    
    this.audio1.loop = false; // We'll handle looping manually for better control
    this.audio1.volume = 0;
    this.audio1.preload = 'auto';
    this.audio2.loop = false; // We'll handle looping manually for better control
    this.audio2.volume = 0;
    this.audio2.preload = 'auto';
    
    // Audio loading event handlers
    this.audio1.addEventListener('loadedmetadata', () => {
        // console.log(`AI Car audio loaded successfully. Duration: ${this.audio1.duration.toFixed(2)}s`);
        this.audioLoaded = true;
        this.setupAudioLoop();
    });
    
    this.audio2.addEventListener('loadedmetadata', () => {
        // console.log(`AI Car audio loaded successfully. Duration: ${this.audio2.duration.toFixed(2)}s`);
        this.audioLoaded = true;
        this.setupAudioLoop();
    });
    
    this.audio1.addEventListener('canplaythrough', () => {
        // console.log("AI Car audio ready to play");
    });
    ...
    this.audio1.addEventListener('error', (e) => {
        // console.error("Error loading AI car siren audio:", e);
        console.error("Audio error details:", {
            code: this.audio1.error?.code,
            message: this.audio1.error?.message
        });
    });
    this.audio2.addEventListener('error', (e) => {
        // console.error("Error loading AI car engine audio:", e);
        // console.error("Audio error details:", {
        //     code: this.audio2.error?.code,
        //     message: this.audio2.error?.message
        // });
    });

    // Handle audio end for manual looping
    this.audio1.addEventListener('ended', () => {
        if (this.isActivated && this.audioLoaded) {
            this.audio1.currentTime = this.audioStartTime;
            this.audio1.play().catch(e => console.warn("Audio restart failed:", e));
        }
    });
    this.audio2.addEventListener('ended', () => {
        if (this.isActivated && this.audioLoaded) {
            this.audio2.currentTime = this.audioStartTime;
            this.audio2.play().catch(e => console.warn("Audio restart failed:", e));
        }
    });
}
...
setupAudioLoop() {
    ...
    if (!this.audio1 || !this.audio2) {
        console.warn('Audio elements not available for loop setup');
        return;
    }
    
    const duration1 = this.audio1.duration;
    const duration2 = this.audio2.duration;
    const loopEnd1 = duration1 - this.audioEndOffset;
    const loopEnd2 = duration2 - this.audioEndOffset;
    
    // Set up seamless looping with null checks
    this.audio1.addEventListener('timeupdate', () => {
        // Check if audio element still exists and is not null
        if (this.audio1 && this.audio1.currentTime !== null && this.audio1.currentTime >= loopEnd1 && this.isActivated) {
            this.audio1.currentTime = this.audioStartTime;
        }
    });
    ...
    this.audio2.addEventListener('timeupdate', () => {
        // Check if audio element still exists and is not null
        if (this.audio2 && this.audio2.currentTime !== null && this.audio2.currentTime >= loopEnd2 && this.isActivated) {
            this.audio2.currentTime = this.audioStartTime;
        }
    });
    
    // console.log(`Audio loop configured: ${this.audioStartTime}s to ${loopEnd1.toFixed(2)}s`);
    // console.log(`Audio loop configured: ${this.audioStartTime}s to ${loopEnd2.toFixed(2)}s`);
}

// Enhanced audio update method - call this in your main update loop
updateAudio() {
    if (!this.audioLoaded || !this.audio1 || !this.audio2) return;
    ...
    // Handle audio playback based on activation state
    if (this.isActivated) {
        this.startAudio(targetVolume);
    } else {
        this.stopAudio();
    }
...
getDistanceToPlayer() {
    if (!this.targetCar || !this.targetCar.car || !this.targetCar.car.chassisBody || 
        ...
        return this.maxDistance; // Return max distance if references are missing
    }
    
    const aiPos = this.car.chassisBody.position;
    const playerPos = this.targetCar.car.chassisBody.position;
...
calculateVolumeFromDistance(distance) {
    if (distance <= this.minDistance) {
        return this.baseVolume; // Full volume when very close
    } else if (distance >= this.maxDistance) {
        return 0; // No sound when too far
    } else {
        // Exponential falloff for more realistic audio distance
        const normalizedDistance = (distance - this.minDistance) / (this.maxDistance - this.minDistance);
        const exponentialFalloff = Math.pow(1 - normalizedDistance, 2); // Quadratic falloff
        return this.baseVolume * exponentialFalloff;
    }
}

startAudio(targetVolume) {
    if (!this.audio1 || !this.audio2 || this.audio1.paused === false || this.audio2.paused === false) return; // Already playing
    
    // Start playing from the configured start time
    this.audio1.currentTime = this.audioStartTime;
    this.audio1.volume = Math.min(targetVolume, 0.1); // Start with low volume to avoid jarring sound
    this.audio2.currentTime = this.audioStartTime;
    this.audio2.volume = Math.min(targetVolume, 0.1); // Start with low volume to avoid jarring sound
    
    // Promise-based play with error handling
    const playPromise1 = this.audio1.play();
    const playPromise2 = this.audio2.play();
    if (playPromise1 !== undefined) {
        playPromise1
            .then(() => {
                console.log("AI Car audio started successfully");
            })
            .catch(error => {
                console.warn("Audio play failed (this is normal if user hasn't interacted):", error.name);
                // Store the intent to play for later
...
stopAudio() {
    if (!this.audio1 || !this.audio2 || this.audio1.paused || this.audio2.paused) return; // Already stopped
    
    // Fade out before stopping
    const fadeOutInterval = setInterval(() => {
        if (this.audio1.volume > 0.05 && this.audio2.volume > 0.05) {
            this.audio1.volume = Math.max(0, this.audio1.volume - 0.05);
            this.audio2.volume = Math.max(0, this.audio2.volume - 0.05);
        } else {
            this.audio1.volume = 0;
            this.audio1.pause();
            this.audio1.currentTime = this.audioStartTime; // Reset for next play
            clearInterval(fadeOutInterval);
            // console.log("AI Car audio stopped");
            this.audio2.volume = 0;
            this.audio2.pause();
            this.audio2.currentTime = this.audioStartTime; // Reset for next play
            clearInterval(fadeOutInterval);
            // console.log("AI Car audio stopped");
        }
...
smoothVolumeTransition(targetVolume) {
    if (!this.audio1 || this.audio1.paused || !this.audio2 || this.audio2.paused) return;
    
    const currentVolume1 = this.audio1.volume;
    const currentVolume2 = this.audio2.volume;
    const volumeDifference1 = targetVolume - currentVolume1;
    const volumeDifference2 = targetVolume - currentVolume2;
    
    // Only adjust if difference is significant (avoid micro-adjustments)
    if (Math.abs(volumeDifference1) > 0.01) {
        // Smooth volume transition (adjust 10% of difference each frame)
        const volumeStep = volumeDifference1 * 0.1;
        const newVolume = Math.max(0, Math.min(1, currentVolume1 + volumeStep));
        ...
        if (this.debugMode && Date.now() % 1000 < 50) { // Log once per second
            const distance = this.getDistanceToPlayer();
            // console.log(`Audio: Distance=${distance.toFixed(1)}, Volume=${newVolume.toFixed(2)}`);
        }
    }
    if (Math.abs(volumeDifference2) > 0.01) {
        // Smooth volume transition (adjust 10% of difference each frame)
        const volumeStep = volumeDifference2 * 0.1;
        const newVolume = Math.max(0, Math.min(1, currentVolume2 + volumeStep));
...
handleUserInteraction() {
    ...
    setWheels() {
      ...
      new CANNON.Vec3(1.2, -0.5, -1.93), // Front right - LOWERED Y position
      new CANNON.Vec3(-1.2, -0.5, -1.93), // Front left - LOWERED Y position
      new CANNON.Vec3(1.2, -0.5, 1.93), // Rear right - LOWERED Y position
      new CANNON.Vec3(-1.2, -0.5, 1.93), // Rear left - LOWERED Y position
        ];
...
checkActivation() {
    ...
    // Check for speed-based activation
    if (this.speedActivationEnabled && !this.isActivated && !this.hitDetected) {
        this.checkSpeedActivation();
    }
...
// 3. Add this new method for speed calculation
calculatePlayerSpeed() {
    if (!this.targetCar || !this.targetCar.car || !this.targetCar.car.chassisBody) return 0;

    const currentTime = Date.now();
    const currentPosition = this.targetCar.car.chassisBody.position;
    ...
    this.currentSpeed = Math.max(0, speed); // Ensure speed is never negative
    return this.currentSpeed;
}

// 4. Add this new method for speed-based activation
checkSpeedActivation() {
    const currentTime = Date.now();
    
    // Don't check speed too frequently
    if (currentTime - this.lastSpeedCheck < this.speedCheckInterval) {
        return;
    }
    ...
    // if (this.debugMode) {
    //     console.log(`Player current speed: ${playerSpeed.toFixed(1)} ${this.speedUnit}, threshold: ${this.speedActivationThreshold}`);
    // }
    
    // Activate if player has reached the speed threshold
    if (playerSpeed >= this.speedActivationThreshold) {
        console.log(`AI Car activated by speed! Player reached ${playerSpeed.toFixed(1)} ${this.speedUnit}`);
        this.hitDetected = true;
        this.activationTime = currentTime;
        
        if (this.debugMode) {
            // console.log(`speed-based activation timer started, will activate in ${this.activationDelay}ms`);
        }
    }
}

// 5. Add this method to enable/disable speed activation (optional utility method)
setSpeedActivation(enabled, threshold = 180, unit = "kmh") {
    this.speedActivationEnabled = enabled;
    this.speedActivationThreshold = threshold;
    this.speedUnit = unit;
    // console.log(`Speed activation ${enabled ? 'enabled' : 'disabled'} with threshold: ${threshold} ${unit}`);
}
...
activateAI() {
    ...
    // Activate police lights if ready
    if (this.policeLightsSetup) {
    ...
    // Play sound with distance-based volume control
playSound() {
    ...
    try {
        ...
        if (this.audio1 && this.audio2) {
            this.audio1.currentTime = this.audioStartTime;
            this.audio2.currentTime = this.audioStartTime;
            this.audio1.volume = this.currentVolume;
            this.audio2.volume = this.currentVolume;
...
pauseSound() {
    try {
        if (this.audio1 && !this.audio1.paused) {
            this.audio1.pause();
            console.log("AI Car audio1 paused");
        }
        if (this.audio2 && !this.audio2.paused) {
            this.audio2.pause();
            console.log("AI Car audio2 paused");
        }
...
cleanup() {
    ...
    if (this.audio1) {
        this.audio1.pause();
        // Need to pass the actual handler functions that were used when adding listeners
        // If you don't have references to the original handlers, you'll need to store them
        // For now, removing the removeEventListener calls since we don't have handler references
        this.audio1.src = ''; // Clear source
        this.audio1 = null;
    }
    if (this.audio2) {
        this.audio2.pause();
    ...
    // Reset speed check timer and speed calculation properties
    this.lastSpeedCheck = 0;
    this.previousPosition = null;
    this.previousTime = 0;
    this.currentSpeed = 0;
...
// Get current audio status (useful for debugging)
getAudioStatus() {
    return {
        audioLoaded: this.audioLoaded,
        isPlaying: this.audio1 && !this.audio1.paused && this.audio2 && this.audio2.paused,
