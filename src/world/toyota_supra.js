...
export default class Toyota_Supra {
    ...
    this.frontLights.forEach((light, index) => {
      if (this.frontLightFlares[index]) {
        ...
        if (flare1.parent === this.chassis) {
          ...
          const offset1 = new THREE.Vector3(-0.7, -0.15, 3.1); // Forward for front lights
          const offset2 = new THREE.Vector3(0.7, -0.15, 3.1); // Further forward
  ...
  setupBrakeLight() {
    ...
    this.chassis.traverse((child) => {
      if (child.name === "Object_29" || child.name === "Object_33") {
    ...
    this.brakeLights.forEach((light, index) => {
      if (this.brakeLightFlares[index]) {
        ...
        if (flare1.parent === this.chassis) {
          ...
          // Update both flares with their respective offsets
          const offset1 = new THREE.Vector3(0.6, 0.1, -3.2);
          const offset2 = new THREE.Vector3(-0.6, 0.1, -3.2);
  ...
  activateReverseLight() {
    if (this.reverseLights && !this.isReversing) {
      ...
      const emissiveColor = new THREE.Color(1, 1, 1).multiplyScalar(0.5);
      const whiteMaterial = new THREE.MeshStandardMaterial({
        color: 0xffffff,
        emissive: emissiveColor,
    ...
    this.reverseLights.forEach((light, index) => {
      if (this.reverseLightFlares[index]) {
        ...
        if (flare1.parent === this.chassis) {
          ...
          // Update both flares with their respective offsets
          const offset1 = new THREE.Vector3(0.35, 0.1, -3.2);
          const offset2 = new THREE.Vector3(-0.35, 0.1, -3.2);
  ...
  loadModels() {
    ...
    gltfLoader.load("./car/toyota_supra.glb", (gltf) => {
    ...
    this.wheels = [];
    for (let i = 0; i < 4; i++) {
      gltfLoader.load("./car/supra_wheels.glb", (gltf) => {
  ...
  setWheels() {
    ...
    for (let i = 0; i < 4; i++) {
      this.car.addWheel({
        ...
        suspensionStiffness: 30, // Reduced for better ground contact
        suspensionRestLength: 0.3, // Shorter for better contact
        ...
        maxSuspensionTravel: 0.3, // Reduced travel
