...
export default class Chevrolet_Corvette {
    ...
    this.frontLights.forEach((light, index) => {
      if (this.frontLightFlares[index]) {
        ...
        if (flare1.parent === this.chassis) {
          ...
          const offset1 = new THREE.Vector3(-1, -0.2, 2.8); // Forward for front lights
          const offset2 = new THREE.Vector3(1, -0.2, 2.8); // Further forward
    ...
    this.brakeLights.forEach((light, index) => {
      if (this.brakeLightFlares[index]) {
        ...
        if (flare1.parent === this.chassis) {
          ...
          // Update both flares with their respective offsets
          const offset1 = new THREE.Vector3(0.8, 0, -3.2);
          const offset2 = new THREE.Vector3(-0.8, 0, -3.2);
  ...
  loadModels() {
    ...
    gltfLoader.load("./car/Chevrolet_Corvette.glb", (gltf) => {
    ...
    this.wheels = [];
    for (let i = 0; i < 4; i++) {
      gltfLoader.load("./car/Chevrolet_Corvette_Wheel.glb", (gltf) => {
