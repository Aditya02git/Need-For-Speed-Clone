...
export default class BMW {
  ...
  setupFrontLight() {
    ...
    this.chassis.traverse((child) => {
      ...
      if (child.name === "Object_1") {
        ...
        const offset1 = new THREE.Vector3(0, 0, 0.8); // Forward offset for front lights
    ...
    this.frontLights.forEach((light, index) => {
      if (this.frontLightFlares[index]) {
        ...
        if (flare1.parent === this.chassis) {
          ...
          const offset1 = new THREE.Vector3(-0.85, -0.4, 3.1); // Forward for front lights
          const offset2 = new THREE.Vector3(0.85, -0.4, 3.1); // Further forward
  ...
  setupBrakeLight() {
    ...
    this.chassis.traverse((child) => {
      if (child.name === "Object_3") {
        ...
        const offset2 = new THREE.Vector3(0, 0, 0.3); // Further back
        flare2.position.copy(localPos).add(offset2);
    ...
    this.brakeLights.forEach((light, index) => {
      if (this.brakeLightFlares[index]) {
        ...
        if (flare1.parent === this.chassis) {
          ...
          // Update both flares with their respective offsets
          const offset1 = new THREE.Vector3(0.9, -0.1, -3.2);
          const offset2 = new THREE.Vector3(-0.9, -0.1, -3.2);
    ...
    this.reverseLights.forEach((light, index) => {
      if (this.reverseLightFlares[index]) {
        ...
        if (flare1.parent === this.chassis) {
          ...
          // Update both flares with their respective offsets
          const offset1 = new THREE.Vector3(0.65, -0.2, -3.2);
          const offset2 = new THREE.Vector3(-0.65, -0.2, -3.2);
  ...
  loadModels() {
    ...
    gltfLoader.load("./car/bmw_m4.glb", (gltf) => {
    ...
    this.wheels = [];
    for (let i = 0; i < 4; i++) {
      gltfLoader.load("./car/bmw_m4_wheel.glb", (gltf) => {
